...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if (!in_array($docStatusFilter, [0, 1, 2, 99])) {
        // if document status is not in (0,1,2,99) thrn return with no data
        echo json_encode([
            'success' => false,
            'data' => [],
        ]);
    ...
    if (in_array((int) $docStatusFilter, [1])) {
        ...
        // LENIENT: For Pending, Suspecious, Re-verified  - AT LEAST ONE document must match the status.
    ...
    // $docStatusFilterQuery = "AND EXISTS (SELECT 1 FROM upload_docs_records udr WHERE udr.RegNoFK = oaf.mobile1 AND udr.DocsVerificationStatus = :docStatusFilter)";
    $params[':docStatusFilter'] = (string) $docStatusFilter;
    ...
    $stmt->bindValue(':docStatusFilter', (string) $params[':docStatusFilter'], PDO::PARAM_STR);
    ;
    ...
    if (!empty($data)) {
        ...
        $docStmt = $pdo->prepare("
            ...
            WHERE udr.RegNoFK IN ($placeholders)
        ");
